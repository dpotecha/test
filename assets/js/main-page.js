...
const servicesPageTabTitles = [...document.querySelectorAll(".services-page__aside-title")];
  ...
  function openServicesFeedBackModal(){
    ...
    if(window.innerWidth < 1024){
      const servicesAsideBot = document.querySelector(".services-page__aside").getBoundingClientRect().bottom;
      const servicesTabs = [...document.querySelectorAll(".services-page__content-tab")];
      const activeServTab = servicesTabs[parseInt(activeServicesTab) - 1]
      const activeServTabTop = activeServTab.getBoundingClientRect().top

      offset = servicesAsideBot - activeServTabTop;
    }
    ...
    if(!servicesFeedBackModal.classList.contains("open")){
      ...
      setTimeout(() => {
        ...
        document.querySelector(".services-page__content-inner").addEventListener('scroll', closeServicesFeedBackModal)
      }, 500);
    }
  }
  function closeServicesFeedBackModal(){
    document.removeEventListener('click', servicesModalClickOutsideHandler)
    document.querySelector(".services-page__content-inner").removeEventListener('scroll', closeServicesFeedBackModal)
...
function changeServicesTab(evt){
  ...
  if(currentTabId !== activeServicesTab){
    ...
    setTimeout(() => {
      getActiveServicesTabContent().activeTab.style.removeProperty('display');
...
function getActiveServicesTabTitle(){
  ...
  function mobAppClose(){
    ...
    const mobItem = document.querySelector(".mobile-apps__item-mobile");
  ...
  function chatBotsClose(){
    ...
    const listMobile = [...document.querySelectorAll(".chat-bots__list-mobile")];
  ...
  function copyrightSmmClose(){
    ...
    gsap.to(items[0], {
      x: window.innerWidth > 1023 ? '78.75vw' : '100vw',
      duration: window.innerWidth > 1023 ? 0.9 : 1,
      delay: window.innerWidth > 1023 ? 0.1 : 0,
      clearProps: true
    })
    gsap.to(items[1], {
      x: window.innerWidth > 1023 ? '87.5vw' : '100vw',
      duration: 1,
      clearProps: true
    })
  }
  ...
  function websiteSecurityClose(){
    ...
    gsap.to(items[0], {
      x: window.innerWidth > 1023 ? '-35vw' : '-100vw',
      duration: 0.85,
      delay: 0.15,
      clearProps: true
    })
  ...
  function erpSystemsClose(){
    ...
    gsap.to(page, {
      xPercent: -100,
      duration: 1,
      delay: window.innerWidth > 1023 ? 0.5 : 0,
      clearProps: true
    })
  }
...
function changeConceptTitle(){
  const titleElement = document.querySelector(".concepts__mob-title")
  const newTitle = conceptsSlides.find(slide => slide.classList.contains("active")).dataset.title
...
function splitContactsTitle(){
  const title = document.querySelector(".contact-us__title");

  const split = new SplitText(title, {type: "chars"})
  const width = split.chars.map(char => char.getBoundingClientRect().width)
